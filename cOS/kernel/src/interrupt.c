...
/*
 * some shorts in this modular
 * ICW: Initialization Command Word
 * OCW: Operational Control Word
 * IRR: Interrupt Request Register
 * IMR: Interrupt Mask Register
 * PR : Priority Resolver
 * ISR: In-Service Regiter
 */

/*
 * use this macro to save the scene
 * similiar with the error_code modular in entry.sx
 * but no need of the error code
 */
#define SAVE_ALL                  \
    "cld;                   \n\t" \
    "pushq    %rax;         \n\t" \
...
/*
 * IRQ_NAME(x)  => IRQ_NAME2(IRQx)
 * IRQ_NAME2(IRQx) => IRQx_interrupt
 */
...
void init_interrupt() {
    ...
    INTERRUPT_DEBUG_PRINT("8259A init\n");
    // 8259A-master	ICW1-4
    io_out8(0x20, 0x11);
    io_out8(0x21, 0x20);
    io_out8(0x21, 0x04);
...
void do_IRQ(uint64 regs, uint64 nr)  // regs:rsp,nr
{
    uint8 x;
    if (unlikely(0x21 == nr)) {
        x = io_in8(0x60);
        INTERRUPT_DEBUG_PRINT("key code:%#08x\n", x);
    }
    /* reset the ISR */
    io_out8(0x20, 0x20);
}
